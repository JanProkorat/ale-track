...
import { useClientNotes, useCreateClientNote, useDeleteClientNote } from 'src/hooks/useClientNotes';

/** Client notes panel — mirrors the brewery NotesPanel, but client notes are
 * REAL backend endpoints (via useDataSource), so this works identically in
 * real and demo sessions with no "unavailable" fallback branch. The client
 * NoteDto only carries {id, text} (no created-date field), so unlike the
 * brewery version there's no timestamp line under each note. */
export function NotesPanel({ clientId, editable }: { clientId: string; editable: boolean }) {
  const { enqueueSnackbar } = useSnackbar();
  const query = useClientNotes(clientId);
  const create = useCreateClientNote(clientId);
  const del = useDeleteClientNote(clientId);
